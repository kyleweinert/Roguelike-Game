...
void io_display(dungeon *d)
{
  ...
  if(obj && d->PC->numItems<=10){
    d->PC->inv[d->PC->numItems] = obj;
    d->PC->numItems += 1;
...
/*
uint32_t io_display_inv(dungeon *d)
{
  pair_t dest;
  int c;
  fd_set readfs;
...
uint32_t io_choose_monster(dungeon *d)
{
  pair_t dest;
  int c;
  fd_set readfs;
  ...
  * not otherwise used.                                                 */
  ...
  do{
    ...
    if (charpair(dest) && charpair(dest) != d->PC) {
      mvprintw(0, 0, charpair(dest)->description);
    }else{
...
void io_handle_input(dungeon *d)
{
  ...
  do {
    ...
    case 'q':
      ...
      break;
    case 'i':
      for(i=0;i<d->PC->numItems;i++){
	message = "";
	message += std::to_string(i) + ": ";
	message += d->PC->inv[i]->get_name();
	tab2 = new char [message.length()+1];
	strcpy (tab2, message.c_str());
	//io_queue_message(tab2);
	mvprintw(i, 0, tab2);
      }
      refresh();
      key = getch();
	...
	if(d->PC->equipment[i]){
	  message = "";
	  message.push_back(intToAlphabet(i));
	  message += ": ";
	  message += d->PC->equipment[i]->get_name();
	  tab2 = new char [message.length()+1];
	  strcpy (tab2, message.c_str());
	  //io_queue_message(tab2);
	  mvprintw(j, 0, tab2);
      ...
      break;
    case 'd':
      mvprintw(0, 0, "Enter inventory slot number:");
      refresh();
      input = getch();
      if(input==27){
	io_display(d);
	...
	break;
      ...
      io_display(d);
      refresh();
      if(isdigit(input) && d->PC->inv[input-'0'] != NULL){
	d->objmap[d->PC->position[dim_y]][d->PC->position[dim_x]] = d->PC->inv[input-'0'];
	for(i=input-'0';i<d->PC->numItems-1;i++){
	  d->PC->inv[i] = d->PC->inv[i+1];
	}
	d->PC->inv[d->PC->numItems - 1] = NULL;
	d->PC->numItems -= 1;
	io_queue_message("Item dropped.");
      }else if(!isdigit(input)){
        mvprintw(0, 0, "Item not dropped. Enter numbers only.");
	refresh();
	key = getch();
      }else{
        mvprintw(0, 0, "No item at that position.");
	refresh();
	key = getch();
      }
      break;
	...
	d->PC->inv[d->PC->numItems - 1] = NULL;
	d->PC->numItems -= 1;
        mvprintw(0, 0, "Item expunged.");
	refresh();
	key = getch();
      }else if(!isdigit(input)){
        mvprintw(0, 0, "Item not expunged. Enter numbers only.");
	refresh();
	key = getch();
      }else{
        mvprintw(0, 0, "No item at that position.");
	...
	break;
      ...
      refresh();
      if(isdigit(input) && d->PC->inv[input-'0'] != NULL){
	message = d->PC->inv[input-'0']->description;
	tab2 = new char [message.length()+1];
	strcpy (tab2, message.c_str());
        mvprintw(0, 0, tab2);
	refresh();
	key = getch();
        ...
        mvprintw(0, 0, "Enter numbers only.");
	refresh();
	key = getch();
      }else{
        mvprintw(0, 0, "No item at that position.");
    ...
    case 'L':
      ...
      break;
    case 't':
      mvprintw(0, 0, "Enter equipment slot letter:");
      refresh();
      input = getch();
      if(input==27){
	io_display(d);
	...
	break;
      ...
      io_display(d);
      refresh();
      if(get_num(i)<=10 && d->PC->equipment[get_num(input) && d->PC->numItems<10]){
	d->PC->inv[d->PC->numItems] = d->PC->equipment[get_num(input)];
	d->PC->speed -= d->PC->equipment[get_num(input)]->speed;
	//i = d->PC->damage->base;
	//d->PC->damage->base = d->PC->equipment[get_num(input)]->addedDam + i;
	d->PC->equipment[get_num(input)] = NULL;
	d->PC->numItems += 1;
	mvprintw(0, 0, "Equipment removed.");
	refresh();
	input = getch();
      }else if(get_num(i)>10){
	mvprintw(0, 0, "Letter not accepted.");
	refresh();
	input = getch();
      }else if(!d->PC->equipment[get_num(input)]){
	mvprintw(0, 0, "No equipment worn in that slot.");
	refresh();
	input = getch();
      }else{
	mvprintw(0, 0, "No room in inventory.");
	refresh();
	input = getch();
      }
      break;
	...
	towear = d->PC->inv[d->PC->numItems - 1];
	d->PC->inv[d->PC->numItems - 1] = NULL;
	d->PC->numItems -= 1;
      }else if(!isdigit(input)){
        mvprintw(0, 0, "Item not worn. Enter numbers only.");
	refresh();
	key = getch();
	break;
      }else{
	...
	break;
      ...
      io_display(d);
      refresh();
      d->PC->speed += towear->speed;
      switch(towear->type){
      case 1: //weapon
	if(d->PC->equipment[0]){
	  d->PC->inv[d->PC->numItems] = d->PC->equipment[0];
	  d->PC->numItems += 1;
	  d->PC->equipment[0] = towear;
	}else{
	  d->PC->equipment[0] = towear;
	}
	break;
      case 2: //offhand
	if(d->PC->equipment[1]){
	  d->PC->inv[d->PC->numItems] = d->PC->equipment[1];
	  d->PC->numItems += 1;
	  d->PC->equipment[1] = towear;
	}else{
	  d->PC->equipment[1] = towear;
	}
	break;
      case 3: //ranged
	if(d->PC->equipment[2]){
	  d->PC->inv[d->PC->numItems] = d->PC->equipment[2];
	  d->PC->numItems += 1;
	  d->PC->equipment[2] = towear;
	}else{
	  d->PC->equipment[2] = towear;
	}
	break;
      case 5: //armor
	if(d->PC->equipment[3]){
	  d->PC->inv[d->PC->numItems] = d->PC->equipment[3];
	  d->PC->numItems += 1;
	  d->PC->equipment[3] = towear;
	}else{
	  d->PC->equipment[3] = towear;
	}
	break;
      case 6: //helmet
	if(d->PC->equipment[4]){
	  d->PC->inv[d->PC->numItems] = d->PC->equipment[4];
	  d->PC->numItems += 1;
	  d->PC->equipment[4] = towear;
	}else{
	  d->PC->equipment[4] = towear;
	}
	break;
      case 7: //cloak
	if(d->PC->equipment[5]){
	  d->PC->inv[d->PC->numItems] = d->PC->equipment[5];
	  d->PC->numItems += 1;
	  d->PC->equipment[5] = towear;
	}else{
	  d->PC->equipment[5] = towear;
	}
	break;
      case 8: //gloves
	if(d->PC->equipment[6]){
	  d->PC->inv[d->PC->numItems] = d->PC->equipment[6];
	  d->PC->numItems += 1;
	  d->PC->equipment[6] = towear;
	}else{
	  d->PC->equipment[6] = towear;
	}
	break;
      case 9: //boots
	if(d->PC->equipment[7]){
	  d->PC->inv[d->PC->numItems] = d->PC->equipment[7];
	  d->PC->numItems += 1;
	  d->PC->equipment[7] = towear;
	}else{
	  d->PC->equipment[7] = towear;
	}
	break;
      case 10: //amulet
	if(d->PC->equipment[8]){
	  d->PC->inv[d->PC->numItems] = d->PC->equipment[8];
	  d->PC->numItems += 1;
	  d->PC->equipment[8] = towear;
	}else{
	  d->PC->equipment[8] = towear;
	}
	break;
      case 4: //light
	if(d->PC->equipment[9]){
	  d->PC->inv[d->PC->numItems] = d->PC->equipment[9];
	  d->PC->numItems += 1;
	  d->PC->equipment[9] = towear;
	}else{
	  d->PC->equipment[9] = towear;
	}
	break;
	...
	if(d->PC->equipment[10]){
	  if(!d->PC->equipment[11]){
	    d->PC->equipment[11] = towear;
	    break;
	  }
	  d->PC->inv[d->PC->numItems] = d->PC->equipment[10];
	  d->PC->numItems += 1;
	  d->PC->equipment[10] = towear;
	  break;
	}else{
	  d->PC->equipment[10] = towear;
	  break;
